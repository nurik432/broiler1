...
function FeedPage() {
    ...
    const handleEditClick = (delivery) => {
        ...
        setEditFormData({ delivery_date: delivery.delivery_date, feed_type: delivery.feed_type, quantity_kg: delivery.quantity_kg });
    };

    const handleUpdate = async (deliveryId) => {
        const { error } = await supabase.from('feed_deliveries').update({ ...editFormData, quantity_kg: Number(editFormData.quantity_kg) }).eq('id', deliveryId);
        if (error) { alert(error.message); }
            ...
            <div className="bg-white rounded-lg shadow-md overflow-x-auto">
                <table className="w-full text-sm text-left">
                    <thead className="bg-gray-50 text-xs text-gray-700 uppercase">
                        <tr><th className="px-6 py-3">Дата прихода</th><th className="px-6 py-3">Тип корма</th><th className="px-6 py-3">Количество (кг)</th><th className="px-6 py-3 text-right">Действия</th></tr>
                    </thead>
                    <tbody>
                        {loading ? ( <tr><td colSpan="4" className="text-center py-4">Загрузка...</td></tr> ) :
                        deliveries.map(d => (
                            <tr key={d.id} className="border-b hover:bg-gray-50">
                                {editingId === d.id ? (
                                    ...
                                    <>
                                        <td className="px-6 py-4">{new Date(d.delivery_date).toLocaleDateString()}</td>
                                        ...
                                        <td className="px-6 py-4 text-right flex gap-4 justify-end"><button onClick={() => handleEditClick(d)} className="font-medium text-blue-600">Редактировать</button><button onClick={() => handleDelete(d.id)} className="font-medium text-red-600">Удалить</button></td>
                                    </>
                                )}
                            </tr>
                        ))}
                         { !loading && deliveries.length === 0 && (<tr><td colSpan="4" className="text-center py-4 text-gray-500">Записей о приходе корма пока нет.</td></tr>) }
                    </tbody>
                </table>
            </div>
        </div>
