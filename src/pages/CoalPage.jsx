...
function CoalPage() {
    ...
    const summary = useMemo(() => {
        return purchases.reduce((acc, p) => {
            ...
            return acc;
        }, { totalKg: 0, totalCost: 0, totalDebt: 0 });
    }, [purchases]);
    ...
    const handleSubmit = async (e) => {
        ...
        const { error } = await supabase.from('coal_purchases').insert([{
            purchase_date: date, supplier_name: supplier,
            quantity_kg: Number(quantity), total_cost: Number(cost),
            user_id: user.id
        }]);
        if (error) { alert(error.message); }
        else {
            setDate(new Date().toISOString().slice(0, 10)); setSupplier(''); setQuantity(''); setCost('');
            await fetchData();
        }
    ...
    const handleAddPayment = async (e) => {
        ...
        const { data: { user } } = await supabase.auth.getUser();
        const { error } = await supabase.from('coal_payments').insert([{
            purchase_id: selectedPurchase.id, payment_date: paymentDate,
            amount: Number(paymentAmount), user_id: user.id
        }]);
        if (error) { alert(error.message); }
        else {
            setPaymentAmount('');
            await fetchData(); // Обновляем основную таблицу
        ...
        <div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                    <div><p className="text-sm text-gray-500">Всего куплено (вес)</p><p className="font-bold text-2xl">{summary.totalKg.toFixed(2)} кг</p></div>
                    <div><p className="text-sm text-gray-500">Всего куплено (сумма)</p><p className="font-bold text-2xl">{formatCurrency(summary.totalCost)}</p></div>
                    <div className="bg-red-50 p-2 rounded-lg"><p className="text-sm font-semibold text-red-700">Общий долг</p><p className="font-bold text-2xl text-red-600">{formatCurrency(summary.totalDebt)}</p></div>
                </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mb-8">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                    ...
                    <div className="md:col-span-2"><label className="block text-sm">Поставщик</label><input type="text" value={supplier} onChange={e => setSupplier(e.target.value)} className="mt-1 w-full p-2 border rounded-md"/></div>
                    ...
                    <div><label className="block text-sm">Общая стоимость</label><input type="number" step="0.01" value={cost} onChange={e => setCost(e.target.value)} required className="mt-1 w-full p-2 border rounded-md"/></div>
                    <button type="submit" disabled={isSubmitting} className="md:col-span-5 w-full bg-indigo-600 text-white p-2 rounded-md">{isSubmitting ? 'Добавление...' : 'Добавить'}</button>
                </form>
            </div>

            <div className="bg-white rounded-lg shadow-md overflow-x-auto">
                <table className="w-full text-sm">
                    <thead className="text-left bg-gray-50 text-xs uppercase"><tr><th className="px-6 py-3">Дата</th><th className="px-6 py-3">Поставщик</th><th className="px-6 py-3">Кол-во</th><th className="px-6 py-3">Стоимость</th><th className="px-6 py-3">Оплачено</th><th className="px-6 py-3">Статус</th><th className="px-6 py-3">Действия</th></tr></thead>
                    <tbody>
                        ...
                        purchases.map(p => (
                            <tr key={p.id} className="border-b">
                                ...
                                <td className="py-2 px-6">{p.supplier_name || '–'}</td>
                                ...
                                <td className="py-2 px-6">{p.balance <= 0 ? (<span className="text-xs font-semibold bg-green-100 text-green-800 px-2 py-1 rounded-full">Оплачено</span>) : (<span className="text-xs font-semibold bg-red-100 text-red-800 px-2 py-1 rounded-full">Долг: {formatCurrency(p.balance)}</span>)}</td>
                                <td className="py-2 px-6"><button onClick={() => openPaymentsModal(p)} className="font-medium text-blue-600 hover:underline">Платежи</button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                   <div className="bg-white rounded-lg shadow-xl w-full max-w-lg">
                        ...
                        <div className="p-6 grid grid-cols-3 gap-4 text-center border-b">
                            <div><p className="text-sm text-gray-500">Общая стоимость</p><p className="font-bold text-lg">{formatCurrency(selectedPurchase.total_cost)}</p></div>
                            <div><p className="text-sm text-gray-500">Оплачено</p><p className="font-bold text-lg text-green-600">{formatCurrency(selectedPurchase.total_paid)}</p></div>
                            <div><p className="text-sm text-gray-500">Остаток долга</p><p className="font-bold text-lg text-red-600">{formatCurrency(selectedPurchase.balance)}</p></div>
                        </div>
                        <div className="p-6">
                            ...
                            {selectedPurchase.balance > 0 && (
                                <div className="text-right -mt-2 mb-4">
                                    <button onClick={() => setPaymentAmount(selectedPurchase.balance)} className="text-xs text-blue-600 hover:underline">внести остаток ({selectedPurchase.balance} TJS)</button>
                                </div>
                            )}
