                        ...
                        <tr key={log.id} className="bg-white border-b hover:bg-gray-50">
                            {editingId === log.id && batch.is_active ? (
                                ...
                                <>
                                    ...
                                    <td className="px-6 py-4 text-right">{batch.is_active && (<div className="flex gap-4 justify-end"><button onClick={() => handleEditClick(log)} className="font-medium text-blue-600">Редактировать</button><button onClick={() => handleDelete(log.id)} className="font-medium text-red-600">Удалить</button></div>)}</td>
                                </>
                            )}
                        </tr>
                    ))}
                    {logs.length === 0 && (<tr><td colSpan="7" className="text-center py-4">Записей нет.</td></tr>)}
                </tbody>
            </table>
        </div>
    );
...
function BatchLogPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        const { error } = await supabase.from('daily_logs').insert([ { batch_id: batchId, log_date: logDate, age: Math.ceil(age / (1000 * 3600 * 24)), mortality: Number(mortality) || 0, medicine_id: medicineId || null, dosage, water_consumption: Number(water) || null, user_id: user.id } ]);
        if (error) { alert(error.message); }
        else { setMortality(''); setMedicineId(''); setDosage(''); setWater(''); await fetchAllBatchData(); }
        ...
        <div>
            ...
            <div className="bg-white rounded-lg shadow-md mt-8">
                ...
                <div className="p-4 md:p-6">
                    {activeTab === 'journal' && (<>{batch.is_active && (<div className="bg-gray-50 p-4 rounded-lg mb-6"><h3 className="text-xl font-semibold mb-4">Добавить запись</h3><form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4"><div><label>Дата</label><input type="date" value={logDate} onChange={e => setLogDate(e.target.value)} required className="mt-1 w-full p-2 border rounded"/></div><div><label>Падеж</label><input type="number" value={mortality} onChange={e => setMortality(e.target.value)} required className="mt-1 w-full p-2 border rounded"/></div><div><label>Вода</label><input type="number" step="0.1" value={water} onChange={e => setWater(e.target.value)} className="mt-1 w-full p-2 border rounded"/></div><div className="md:col-span-2"><label>Лекарство</label><select value={medicineId} onChange={e => setMedicineId(e.target.value)} className="mt-1 w-full p-2 border rounded bg-white"><option value="">-- Не выбрано --</option>{medicines.map(med => <option key={med.id} value={med.id}>{med.name}</option>)}</select></div><div><label>Доза</label><input type="text" value={dosage} onChange={e => setDosage(e.target.value)} className="mt-1 w-full p-2 border rounded"/></div><button type="submit" disabled={isSubmitting} className="md:col-span-3 w-full py-2 text-white bg-indigo-600 rounded">{isSubmitting ? '...' : 'Добавить'}</button></form></div>)}<JournalTable logs={logs} batch={batch} medicines={medicines} fetchData={fetchAllBatchData} /></>)}
